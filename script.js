...
// Imagem promocional só em zonas específicas do cardápio — não no menu todo.
// Guarnições & Sopas fica sem imagem de propósito (nenhum visual promocional combina).
const PROMOS = {
  'Churrasco': 'promo-churrasco.jpg',
  'Fast Food & Snacks': 'promo-fastfood.jpg',
  'Grelhada Mista': 'promo-grelhada.jpg',
};

let activeCategory = 'Todos';
let cart = {}; // { itemId: qty }
let fulfillmentMode = 'entrega'; // 'entrega' | 'levantamento'
...
function renderMenu(){
  ...
  catsToShow.forEach(cat=>{
    ...
    if(PROMOS[cat]){
      html += `<div class="promo-card" style="background-image:url('${PROMOS[cat]}')"></div>`;
    }
  });
  grid.innerHTML = html;
...
const modeDeliveryBtn = document.getElementById('modeDelivery');
...
function setFulfillmentMode(mode){
  ...
  modeDeliveryBtn.classList.toggle('active', mode==='entrega');
  modePickupBtn.classList.toggle('active', mode==='levantamento');
  addressField.classList.toggle('hidden', mode!=='entrega');
}
modeDeliveryBtn.addEventListener('click', ()=> setFulfillmentMode('entrega'));
modePickupBtn.addEventListener('click', ()=> setFulfillmentMode('levantamento'));
setFulfillmentMode('entrega');
...
document.getElementById('checkoutBtn').addEventListener('click', ()=>{
  ...
  if(fulfillmentMode==='entrega' && !addressInput.value.trim()){
    alert('Por favor indique o endereço de entrega antes de finalizar.');
    addressInput.focus();
    return;
  }
  ...
  if(fulfillmentMode==='entrega'){
    fulfillmentBlock =
`Modo: Entrega\n` +
`Endereço: ${addressInput.value.trim()}\n\n` +
`Podem confirmar o tempo estimado de entrega e o valor da entrega, por favor?`;
  } else {
    fulfillmentBlock =
`Modo: Levantamento no local\n\n` +
`Podem confirmar em quanto tempo fica pronto para levantar, por favor?`;
  }

  const message =
`Olá VanilC! Gostaria de fazer o seguinte pedido:\n\n` +
lines.join('\n') +
`\n\nTotal dos itens: ${fmt(total)}\n\n` +
fulfillmentBlock +
`\n\nObrigado!`;
